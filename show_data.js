...
const readAndEmitData = () => {
    fs.readFile(logFilePath, 'utf8', (err, data) => {
        ...
        const records = lines.map(line => {
            const [time, elapsed, temperature, state] = line.split(',').map(s => s.trim());
            return { time, elapsed, temperature, state };
        });

        // Emit the data to connected clients
        io.emit('data', records);
        
        // Get latest temp
        const latestRec = records[records.length - 1];
        io.emit('temperature', latestRec.temperature);
    });
};
